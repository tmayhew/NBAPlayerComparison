...
userinput2 = "Michael Jordan"
html1 = get_htmlnba(userinput1)
html2 = get_htmlnba(userinput2)
...
player2d = scrape_nbadf(html2, userinput2)

names1 = colnames(player1d);names2 = colnames(player2d);col1 = which(names1 %in% names2);col2 = which(names2 %in% names1)
...
player1d = player1d %>% mutate(GmSc = 2*PTS + (0.4*FG) - (0.7*FGA) - (0.4*(FTA - FT)) + TRB + (0.7*AST) - (0.4*PF)) %>% arrange(desc(GmSc))
player2d = player2d %>% mutate(GmSc = 2*PTS + (0.4*FG) - (0.7*FGA) - (0.4*(FTA - FT)) + TRB + (0.7*AST) - (0.4*PF)) %>% arrange(desc(GmSc))

# Best X Year Statistics
primedf = function(player1d, player2d, i = 1){
  ...
  t = formattable(rbind.data.frame(p1p1, p2p1), list(`PTS/G` = greater_bold,`TRB/G` = greater_bold,`AST/G` = greater_bold,`TS%` = greater_bold,`FT%` = greater_bold,PER = greater_bold,WS = greater_bold))
  return(t)
}
primedf(player1d, player2d, i = 2)
...
scrape_accolades = function(html_input, user_player){
  ...
  for (i in 1:length(acc)){
    ...
    if (length(s) < 5){
      ...
      if (paste(s[(length(s)-2):length(s)], collapse = "") == "MVP" & paste(s[(length(s)-4):length(s)], collapse = "") != "s MVP"){
...
acc1 = scrape_accolades(html1, userinput1)
acc2 = scrape_accolades(html2, userinput2)
accdf = rbind.data.frame(acc1, acc2)
names(accdf) = c("Player", "Hall of Fame Induction", "All Star Selections", "Rookie Of the Year", "Most Valuable Player", "Finals Most Valuable Player", "Scoring Titles", "Championships", "All-NBA Selections", "All-Defensive Team Selections", "All-Rookie Team")
accdf = t(accdf)
...
player1sum = player1car %>% summarise(Player = acc1$player,MP = sum(MP),FG = sum(FG),FGA = sum(FGA),FT = sum(FT),FTA = sum(FTA),TRB = sum(TRB),AST = sum(AST),PTS = sum(PTS))
...
player2sum = player2car %>% summarise(Player = acc2$player,MP = sum(MP),FG = sum(FG),FGA = sum(FGA),FT = sum(FT),FTA = sum(FTA),TRB = sum(TRB),AST = sum(AST),PTS = sum(PTS))
...
# Selected Statistic Plot
compdf %>% ggplot(aes(x = Yr, y = `PER`)) + 
  geom_line(aes(color = Player)) + 
  geom_point(aes(color = Player)) +
  scale_x_continuous("Year of Career") + 
  theme_classic() + scale_color_manual("",values = c("black", "grey65")) + 
  theme(legend.position = c(0.90,0.90)) + ggtitle("Career Progression")
