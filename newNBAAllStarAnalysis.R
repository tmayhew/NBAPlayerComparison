...
lgAvgG = max(df$G)
fdf = df %>% transmute(Player, Yr, x3PAVG = ((X3P. - lgAvg3P)*(X3PA))/G,x2PAVG = ((X2P. - lgAvg2P)*(X2PA))/G,PTS = PTS/G,TRB = TRB/G,AST = AST/G,PER = PER*(MP/(lgAvgMP))*(G/lgAvgG),WS = WS/G,OWS = OWS/G,DWS = DWS/G,TS = TS.,FTr = FTr)
...
finaldf %>% gather("stat", "value", -Player, -allstar) %>% ggplot(aes(x = as.factor(allstar), y = value)) + geom_hline(yintercept = 0, alpha = I(1/3), linetype = "dashed") + geom_boxplot(aes(fill = stat)) + theme_bw() + ggtitle(paste(year)) + scale_x_discrete("All Star") + scale_y_continuous("")
